...
AgregarVariables  <- function( dataset )
{
  ...
  # Pagado - Pago mÌn = Dif entre lo pagado que excede el mÌnimo
  ...
  ### Disclaimer a partir de ac· es un ROBO literal a los compaÒeros DRAGO y LARRETA 
  ### que muy amablemente compartieron su cÛdigo en GIT, los invito a ROBAR con toda libertad
  ### mi cÛdigo.
  
  #CLASIFICACION DE LOS CLIENTES (BRAIAN)## 
  #1 al 3 donde 1 son los que tienen antiguedad menor a 12 meses (BRAIAN)##
  dataset[ , varnueva_80 := cliente_antiguedad ]
  dataset[ , varnueva_81 := ifelse(cliente_antiguedad<=12,1,ifelse(cliente_antiguedad>=36,3,2))]
  
  #LIQUIDEZ DEL CLIENTE (disp. inmediata de efectivo por parte de el cliente) (BRAIAN)##
  dataset[ , varnueva_82 := rowSums( cbind( mcuenta_corriente_adicional,  mcuenta_corriente) , na.rm=TRUE ) ]
  dataset[ , varnueva_83 := rowSums( cbind( mcaja_ahorro_adicional,mcaja_ahorro ) , na.rm=TRUE ) ]
  dataset[ , varnueva_84 := rowSums( cbind( liquid_cc,  liquid_ca) , na.rm=TRUE ) ]
  dataset[ , varnueva_85 := rowSums( cbind( liquid_tot/mcuentas_saldo) , na.rm=TRUE ) ]
  
  #ACREDITACIONES / MONTOS DE ACREDITACIONES DE HABERES y LIMITE DE COMPRA (BRAIAN)##
  dataset[ , varnueva_86 := rowSums( cbind( mpayroll,  mpayroll2) , na.rm=TRUE ) ]
  dataset[ , varnueva_87:= ifelse(acred_m == 0, NA, mv_mlimitecompra / acred_m) ]
  
  #PRESTAMOS (BRAIAN)
  dataset[ , varnueva_88 := rowSums( cbind( cprestamos_personales,  cprestamos_prendarios, cprestamos_hipotecarios) , na.rm=TRUE ) ]
  dataset[ , varnueva_89 := rowSums( cbind( mprestamos_personales,  mprestamos_prendarios, mprestamos_hipotecarios) , na.rm=TRUE ) ]
  dataset[ , varnueva_90:= ifelse(cprestamos == 0, NA, mprestamos_tot / cprestamos) ]
  
  #INVERSIONES - Se decide incluir al PF como inversion (BRAIAN)
  dataset[ , varnueva_91 := rowSums( cbind( cinversion1, cinversion2, cplazo_fijo) , na.rm=TRUE ) ]
  dataset[ , varnueva_92 := rowSums( cbind( mplazo_fijo_dolares,  mplazo_fijo_pesos) , na.rm=TRUE ) ]
  dataset[ , varnueva_93 := rowSums( cbind( minversion1_pesos,  minversion1_dolares, minversion2) , na.rm=TRUE ) ]
  dataset[ , varnueva_94 := rowSums( cbind( sum_pfijos,  sum_invers) , na.rm=TRUE ) ]
  dataset[ , varnueva_95:= ifelse(cant_invers == 0, NA, tot_invers / cant_invers) ]
  
  #TENENCIA: INVERSION/(INV + LIQUIDEZ) (% de inversiones sobre tenencia total) (BRAIAN)
  dataset[ , varnueva_96 := rowSums( cbind( tot_invers,  liquid_tot) , na.rm=TRUE ) ]
  dataset[ , varnueva_97 := ifelse(tenencia == 0, NA, tot_invers / tenencia) ] 
  
  #COMISIONES - (BRAIAN)
  #Se plantea relacion entre liquidez y comisiones y entre acreditaciones y comisiones
  dataset[ , varnueva_98 := rowSums( cbind( mcomisiones_mantenimiento,  mcomisiones_otras) , na.rm=TRUE ) ]
  dataset[ , varnueva_99 := ifelse(liquid_tot == 0, NA,comisiones_tot/ liquid_tot) ] 
  dataset[ , varnueva_100 := ifelse(acred_m == 0, NA, comisiones_tot/ acred_m ) ] 
  ...
  dataset[ , varnueva_101 := cmobile_app_trx/ (ctrx_quarter/3) ]
  dataset[ , varnueva_102 := cmobile_app_trx/ (chomebanking_trx) ]
  
  #RENTABILIDAD (Braian)
  dataset[ , varnueva_103 := mrentabilidad/ mrentabilidad_annual ]
